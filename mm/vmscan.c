 ...
 */
#ifdef BYPASS_SWAP_GARBAGE
static unsigned long shrink_page_list_profiling(struct list_head *page_list,
				      ...
				      bool skip_reference_check)
{
	...
	while (!list_empty(page_list)) {
		...
		enum jvm_heap_flag adc_advise = OUT_JVM_HEAP;
...
#ifdef ADC_ADVISE_FREE_BYPASS_ALLOC_SWAP_ENTRY
		if (PageAnon(page) && page_mapped(page) && PageSwapBacked(page)) {
			adc_advise = adc_check_page_freed(page, &vaddr, &vma, &bitmap_entry);
			if (adc_advise == IN_JVM_HEAP_FREE) {
				ClearPageSwapBacked(page);
			    // printk(KERN_INFO "YYZ ADC_ADVISE_FREE_BYPASS_ALLOC_SWAP_ENTRY: adc_advise == IN_JVM_HEAP_FREE, ClearPageSwapBacked(page)\n");
			}
		}
#endif // ADC_ADVISE_FREE_BYPASS_ALLOC_SWAP_ENTRY
		/*
		...
		if (page_mapped(page)) {
			...
			// if (adc_advise == IN_JVM_HEAP_FREE)
			// 	printk(KERN_INFO "YYZ ADC_ADVISE_FREE_BYPASS_ALLOC_SWAP_ENTRY: adc_advise == IN_JVM_HEAP_FREE\n");
#endif // ADC_ADVISE_FREE_BYPASS_ALLOC_SWAP_ENTRY

#ifdef ADC_ADVISE_FREE_BYPASS_ALLOC_SWAP_ENTRY
	#ifndef ADC_ADVISE_SWAPOUT_SKIP_WALK_RMAP_FOR_UNMAP
		if (!try_to_unmap(page, flags))
	#else
		if (!try_to_unmap_addr(page, flags, vaddr, vma, bitmap_entry))
	#endif
#else
		if (!try_to_unmap_profiling(page, flags, &adc_advise))
#endif
		...
		if (PageDirty(page))
#else
		if (PageDirty(page) && (adc_advise != IN_JVM_HEAP_FREE))
#endif
		{
			/*
			...
			case PAGE_ACTIVATE:
				goto activate_locked;
			case PAGE_SUCCESS:
				if (unlikely(adc_advise == IN_JVM_HEAP_FREE)) {
#ifdef ADC_ADVISE_FREE_BYPASS_ALLOC_SWAP_ENTRY
					printk("YYZ BUG: dirty page should not be runtime free.\n");
#endif
				}
				if (PageWriteback(page))
					goto keep;
...
static unsigned long shrink_page_list(struct list_head *page_list,
				      ...
				      bool skip_reference_check)
{
	return shrink_page_list_profiling(page_list, pgdat, sc, ttu_flags, stat, skip_reference_check);
}
#else
static unsigned long shrink_page_list(struct list_head *page_list,
				      struct pglist_data *pgdat,
